...
--[[
    ZOOM TO MOUSE OPTIMIZE cho OBS Studio
    Tác giả bản cải tiến và sửa lỗi : nguyenquocanhz
    ...
    function refresh_sceneitem(find_newest)
                    ...
                    if scene_source ~= nil then
                        local function find_scene_item_by_name(root_scene)
                            ...
                            while #queue > 0 do
                                ...
                                log("Đang kiểm tra scene '" .. obs.obs_source_get_name(obs.obs_scene_get_source(s)) .. "'")
        ...
        if is_non_display_capture then
            if not use_monitor_override then
                log("ERROR: Đã chọn nguồn zoom không phải là nguồn capture màn hình.\n" ..
                    "       Bạn PHẢI bật 'Set manual source position' và đặt các giá trị override đúng cho kích thước và vị trí.")
            end
        end
        ...
        if sceneitem ~= nil then
            ...
            if not source then
                log("ERROR: Không thể lấy nguồn cho sceneitem (" .. source_name .. ")")
            end
            ...
            if source_width == 0 or source_height == 0 then
                log("ERROR: Có gì đó sai sai khi xác định kích thước nguồn." ..
                    "       Hãy thử sử dụng tùy chọn 'Set manual source position' và thêm các giá trị override")
            ...
            if sceneitem_info.bounds_type == obs.OBS_BOUNDS_NONE then
                ...
                log("WARNING: Đang tồn tại transform không phải là bounding box. Điều này có thể gây ra vấn đề với việc zoom.\n" ..
                    "         Các cài đặt đã được tự động chuyển đổi thành một transform scaling bounding box thay thế.\n" ..
                    "         Nếu bạn gặp sự cố với bố cục của mình, hãy cân nhắc việc làm cho transform sử dụng một bounding box một cách thủ công.")
            end
                        ...
                        if name ~= CROP_FILTER_NAME and name ~= "temp_" .. CROP_FILTER_NAME then
                            ...
                            if settings ~= nil then
                                if not obs.obs_data_get_bool(settings, "relative") then
                                    ...
                                    log("Đang tồn tại filter crop/pad tương đối (" ..
                                        name ..
                                        "). Đang áp dụng cài đặt " .. format_table(zoom_info.source_crop_filter))
                                else
                                    log("WARNING: Đang tồn tại filter crop/pad không tương đối (" .. name .. ").\n" ..
                                        "         Điều này sẽ gây ra vấn đề với việc zoom. Vui lòng chuyển sang cài đặt tương đối.")
                                end
                                obs.obs_data_release(settings)
                            end
                        end
            ...
            if not found_crop_filter and (sceneitem_crop_orig.left ~= 0 or sceneitem_crop_orig.top ~= 0 or sceneitem_crop_orig.right ~= 0 or sceneitem_crop_orig.bottom ~= 0) then
                ...
                log("WARNING: Đang tồn tại transform crop. Điều này có thể gây ra vấn đề với việc zoom.\n" ..
                    "         Các cài đặt đã được tự động chuyển đổi thành một filter crop/pad tương đối thay thế.\n" ..
                    "         Nếu bạn gặp sự cố với bố cục của mình, hãy cân nhắc việc làm cho filter sử dụng một bounding box một cách thủ công.")
    ...
    function on_toggle_zoom(pressed)
        if pressed then
            -- Auto-refresh sceneitem nếu chưa được khởi tạo (fix hotkey không hoạt động)
            if sceneitem == nil and source_name ~= nil and source_name ~= "" and source_name ~= "obs-zoom-to-mouse-none" then
                log("Sceneitem is nil when hotkey pressed, attempting refresh...")
                refresh_sceneitem(true)
            end

            -- Nếu vẫn không tìm được sceneitem thì không làm gì
            if sceneitem == nil then
                log("WARNING: Không thể zoom - không tìm thấy nguồn zoom hợp lệ trong cảnh hiện tại.\n" ..
                    "         Đảm bảo rằng Zoom Source đã được chọn và là một phần của cảnh hiện tại.")
                return
            end
            ...
            if zoom_state == ZoomState.ZoomedIn or zoom_state == ZoomState.None then
                if zoom_state == ZoomState.ZoomedIn then
                    ...
                    if is_following_mouse then
                        ...
                        log("Đang theo dõi chuột is off (due to zoom out)")
                    end
                else
                    log("Zooming in")
    ...
    function on_timer()
        -- Safety guard: dừng timer nếu sceneitem hoặc crop_filter bị release giữa chừng
                    ...
                    if not skip_frame then
                        ...
                        if locked_center ~= nil then
                            ...
                            if math.abs(diff.x) > track.x or math.abs(diff.y) > track.y then
                                ...
                                log("Đã thoát khỏi khu vực khóa - tiếp tục theo dõi chuột")
                            end
                        end
                        ...
                        if locked_center == nil and (zoom_target.crop.x ~= crop_filter_info.x or zoom_target.crop.y ~= crop_filter_info.y) then
                            ...
                            if is_following_mouse and locked_center == nil and locked_last_pos ~= nil then
                                ...
                                if (lock and use_follow_auto_lock) or (diff.x <= follow_safezone_sensitivity and diff.y <= follow_safezone_sensitivity) then
                                    ...
                                    log("Cursor đã dừng. Dã khóa tại " .. locked_center.x .. ", " .. locked_center.y)
                                end
                            end
                        end
                    end
